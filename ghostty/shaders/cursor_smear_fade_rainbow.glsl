...
vec4 getRainbowColor(vec2 uv, float hueShift) {
    float hue = fract(uv.y + uv.x * 0.35 + hueShift);
    return vec4(hsv2rgb(vec3(hue, 0.85, 1.0)), 1.0);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec4 trailColor = getRainbowColor(screenUv, iTime * 0.15);
    vec4 cursorColor = getRainbowColor(cursorLocalUv, iTime * 0.2);
